                    ...
                    if(completed_steps < 2){
                        if(status != 2){
                            html += `<a href="${base}/create-business/${encodedId}" class="btn btn-sm btn-primary btn-sm" title="Business" target="_blank">
                                        <i class="fa-solid fa-briefcase" style="margin-right:2px;"></i>Add Business
                                    </a>`;
                        }
                    } else {
                        if(status != 2){
                            html += `<br><br><a href="${base}/create-business/${encodedId}" class="btn btn-sm btn-primary btn-sm" title="Business" target="_blank">
                                        <i class="fa-solid fa-briefcase" style="margin-right:2px;"></i>Edit Business
                                    </a>`;
                        }
                        html += `<br><br><a href="${base}/profile/${encodedId}" class="btn btn-sm btn-info btn-sm" title="Profile" target="_blank">
                                    <i class="fa-solid fa-info-circle" style="margin-right:2px;"></i>View Profile
                                </a>`;
                    }
                    if(status == 0){
                        html += `<br><br><a href="${base}/verify-otp/${encodedId}" class="btn btn-sm btn-warning btn-sm" title="Verify OTP">
                                    <i class="fa-solid fa-key" style="margin-right:2px;"></i>Verify OTP
                                </a>`;
                    } else if(status == 1){
                        html += `<br><br><a href="${base}/change-status/${encodedId}/4" class="btn btn-success btn-sm me-1" title="Click to Verify">
                                    <i class="fa-solid fa-check" style="margin-right:2px;"></i> Click to Verify
                                </a>`;
                        html += `<br><br><a href="${base}/change-status/${encodedId}/2" class="btn btn-danger btn-sm me-1" title="Click to Decline">
                                    <i class="fas fa-times" style="margin-right:2px;"></i> Click to Decline
                                </a>`;
                    } else if(status == 2){
                        html += `<br><br><span class="badge bg-danger"><i class="fas fa-times" style="margin-right:2px;"></i> DECLINE</span>`;
                        html += `<br><br><a href="${base}/delete/${encodedId}" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure?')" title="Delete">
                            <i class="fa-solid fa-trash" style="margin-right:2px;"></i> Delete
                        </a>`;
                    } else if(status == 4){
                        html += `<br><br><span class="badge bg-success"><i class="fa-solid fa-check" style="margin-right:2px;"></i> VERIFIED</span>`;
                    }
                }
