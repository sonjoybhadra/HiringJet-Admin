...
class BenefitController extends Controller
{
    ...
    {        
        $this->data = array(
            ...
            'controller_route'  => 'benefit',
            'primary_key'       => 'id',
            'table_name'        => 'perk_benefits',
        );
    }
    /* list */
        public function list(){
            ...
            $page_name                      = 'benefit.list';
            $data['rows']                   = PerkBenefit::where('status', '!=', 3)->orderBy('id', 'DESC')->get();
    ...
    /* edit */
        public function edit(Request $request, $id){
            ...
            $page_name                      = 'benefit.add-edit';
            $data['row']                    = PerkBenefit::where('id', '=', $id)->first();
            if($request->isMethod('post')){
                $postData = $request->all();
                $rules = [
                    'name'           => 'required',
                ...
                if($this->validate($request, $rules)){
                    ...
                    PerkBenefit::where($this->data['primary_key'], '=', $id)->update($fields);
                    /* user activity */
                        $activityData = [
                            'user_email'        => session('user_data')['email'],
                            'user_name'         => session('user_data')['name'],
