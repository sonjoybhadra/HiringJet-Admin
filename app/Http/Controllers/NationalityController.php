...
class NationalityController extends Controller
{
    ...
    /* list */
        public function list(){
            ...
            $page_name                      = 'nationality.list';
            $data['rows']                   = Nationality::where('status', '!=', 3)->orderBy('id', 'DESC')->get();
    ...
    /* add */
        public function add(Request $request){
            ...
            $page_name                      = 'nationality.add-edit';
            $data['row']                    = [];
            echo $this->admin_after_login_layout($title,$page_name,$data);
        }
    /* add */
    /* edit */
        public function edit(Request $request, $id){
            ...
            $page_name                      = 'nationality.add-edit';
            $data['row']                    = Nationality::where('id', '=', $id)->first();
            if($request->isMethod('post')){
                $postData = $request->all();
                $rules = [
                    'name'           => 'required',
                ...
                if($this->validate($request, $rules)){
                    ...
                    Nationality::where($this->data['primary_key'], '=', $id)->update($fields);
                    /* user activity */
                        $activityData = [
                            'user_email'        => session('user_data')['email'],
                            'user_name'         => session('user_data')['name'],
