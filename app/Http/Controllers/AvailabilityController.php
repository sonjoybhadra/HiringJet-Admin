...
class AvailabilityController extends Controller
{
    ...
    /* list */
        public function list(){
            ...
            $page_name                      = 'availability.list';
            $data['rows']                   = Availability::where('status', '!=', 3)->orderBy('id', 'DESC')->get();
    ...
    /* add */
        public function add(Request $request){
            ...
            $page_name                      = 'availability.add-edit';
            $data['row']                    = [];
            echo $this->admin_after_login_layout($title,$page_name,$data);
        }
    /* add */
    /* edit */
        public function edit(Request $request, $id){
            ...
            $page_name                      = 'availability.add-edit';
            $data['row']                    = Availability::where('id', '=', $id)->first();
            if($request->isMethod('post')){
                $postData = $request->all();
                $rules = [
                    'name'           => 'required',
                ...
                if($this->validate($request, $rules)){
                    ...
                    Availability::where($this->data['primary_key'], '=', $id)->update($fields);
                    /* user activity */
                        $activityData = [
                            'user_email'        => session('user_data')['email'],
                            'user_name'         => session('user_data')['name'],
