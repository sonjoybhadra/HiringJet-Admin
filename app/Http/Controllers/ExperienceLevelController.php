...
class ExperienceLevelController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->data = array(
            ...
            'controller_route'  => 'experience-level',
            'primary_key'       => 'id',
    ...
    /* list */
        public function list(){
            ...
            $page_name                      = 'experience-level.list';
    ...
    /* add */
        public function add(Request $request){
            ...
            $page_name                      = 'experience-level.add-edit';
            $data['row']                    = [];
            $data                           = $this->siteAuthService ->admin_after_login_layout($title,$page_name,$data);
            return view('maincontents.' . $page_name, $data);
        }
    ...
    /* edit */
        public function edit(Request $request, $id){
            ...
            $page_name                      = 'experience-level.add-edit';
            $data['row']                    = ExperienceLevel::where('id', '=', $id)->first();
            if($request->isMethod('post')){
                $postData = $request->all();
                $rules = [
                    'name'           => 'required',
                ...
                if($this->validate($request, $rules)){
                    ...
                    ExperienceLevel::where($this->data['primary_key'], '=', $id)->update($fields);
                    /* user activity */
                        $activityData = [
                            'user_email'        => session('user_data')['email'],
                            'user_name'         => session('user_data')['name'],
