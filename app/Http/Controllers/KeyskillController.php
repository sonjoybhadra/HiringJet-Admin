...
class KeyskillController extends Controller
{
    ...
    {        
        $this->data = array(
            ...
            'controller_route'  => 'keyskill',
            'primary_key'       => 'id',
            'table_name'        => 'keyskills',
        );
    }
    /* list */
        public function list(){
            ...
            $page_name                      = 'keyskill.list';
            $data['rows']                   = Keyskill::where('status', '!=', 3)->orderBy('id', 'DESC')->get();
    ...
    /* edit */
        public function edit(Request $request, $id){
            ...
            $page_name                      = 'keyskill.add-edit';
            $data['row']                    = Keyskill::where('id', '=', $id)->first();
            if($request->isMethod('post')){
                $postData = $request->all();
                $rules = [
                    'name'           => 'required',
                ...
                if($this->validate($request, $rules)){
                    ...
                    Keyskill::where($this->data['primary_key'], '=', $id)->update($fields);
                    /* user activity */
                        $activityData = [
                            'user_email'        => session('user_data')['email'],
                            'user_name'         => session('user_data')['name'],
