...
class OnlineProfileController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->data = array(
            ...
            'controller_route'  => 'online-profile',
            'primary_key'       => 'id',
            'table_name'        => 'online_profiles',
        );
    }
    /* list */
    ...
    /* edit */
        public function edit(Request $request, $id){
            ...
            $page_name                      = 'online-profile.add-edit';
            $data['row']                    = OnlineProfile::where('id', '=', $id)->first();
            if($request->isMethod('post')){
                $postData = $request->all();
                $rules = [
                    'name'           => 'required',
                ...
                if($this->validate($request, $rules)){
                    ...
                    OnlineProfile::where($this->data['primary_key'], '=', $id)->update($fields);
                    /* user activity */
                        $activityData = [
                            'user_email'        => session('user_data')['email'],
                            'user_name'         => session('user_data')['name'],
