...
class CommonController extends BaseApiController
{
    ...
    public function get_masters_by_params(Request $request){
        ...
        $common_resource_array = [
            ...
            'contract_type'=> $this->get_contractType(1)
        ];
    ...
    {
        $list = Country::selectRaw('currency_code, currency_code as name, MIN(id) as id, MIN(country_short_code) as country_short_code')
                        ->where('status', 1)
                        ->where('currency_code','<>','')
    ...
    public function get_contractType($res = '')
    {
        $list = ContractType::select('id', 'name')->where('status', 1)->get();
        if($res != ''){
            return $list;
        }else{
            return $this->sendResponse(
    ...
    public function get_homepage(){
        ...
        $live_jobs = PostJob::where('status', 1)->where('created_at', '>=', date('Y-m-d', strtotime('-'.env('LIVE_JOBS_SHOW_DAYS').' days')))
                                                ->get();
        $list->live_jobs= $live_jobs->count(); //PostJob::where('created_at', '>=', date('Y-m-d', strtotime('-30 days')))->count();
        ...
        $list->new_jobs= PostJob::where('created_at', '>=', date('Y-m-d', strtotime('-'.env('NEW_JOBS_SHOW_DAYS').' days')))
                                ->where('status', 1)
                                ->count();
