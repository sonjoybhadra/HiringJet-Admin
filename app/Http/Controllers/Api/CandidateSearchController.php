...
class CandidateSearchController extends BaseApiController{
    ...
    public function searchCandidates(Request $request){
        ...
        try {
            $result = DB::table('users')
                ...
                // ✅ tag names subquery (added back)
                ->leftJoin(DB::raw("(
                    SELECT tjm.jobseeker_id,
                           STRING_AGG(DISTINCT et.tag_name, ', ') AS tag_names
                    FROM tag_jobseeker_mappings tjm
                    JOIN employer_tags et ON tjm.tag_id = et.id
                    GROUP BY tjm.jobseeker_id
                ) as tagset"), 'users.id', '=', 'tagset.jobseeker_id')
  ...
  public function previewCV($id)
    ...
    public function sendEmail(Request $request){
        ...
        try {
            Mail::to($request->email)->send(new CVJobEmail($message, $subject));
            ...
            return $this->sendError('Email failed: ' . $e->getMessage());
        }
