...
use App\Models\UserAccountSetting; // Jobseeker account settings

class EmployerJobseekerController extends BaseApiController
{
    ...
    /**
     * Implementing Job Seeker Block Employers (All jobseekers list who blocked this emplyee)
    */
    public function getBlockedByJobseeker(Request $request)
    {
        try {
            $jobseekers = UserAccountSetting::whereJsonContains('value', auth()->user()->id)
                                            ->where('key', 'hide-my-profile')
                                            ->get()
                                            ->pluck('user_id')->toArray();
            $list = [];
            if(count($jobseekers) > 0){
                $list = User::select('id', 'first_name', 'last_name', 'email')
                    ->whereIn('id', $jobseekers)->get();
            }
            return $this->sendResponse(
                $list,
                'Blocked by jobseekers list'
            );
        } catch (\Exception $e) {
