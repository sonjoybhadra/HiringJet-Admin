...
class VisaStatusController extends Controller
{
    ...
    {        
        $this->data = array(
            'title'             => 'Visa Status For Current Location',
            'controller'        => 'VisaStatusController',
            'controller_route'  => 'visa-status',
            'primary_key'       => 'id',
            'table_name'        => 'visa_statuses',
        );
    }
    /* list */
        public function list(){
            ...
            $page_name                      = 'visa-status.list';
            $data['rows']                   = VisaStatus::where('status', '!=', 3)->orderBy('id', 'DESC')->get();
    ...
    /* add */
        public function add(Request $request){
            ...
            $page_name                      = 'visa-status.add-edit';
            $data['row']                    = [];
            echo $this->admin_after_login_layout($title,$page_name,$data);
        }
    /* add */
    /* edit */
        public function edit(Request $request, $id){
            ...
            $page_name                      = 'visa-status.add-edit';
            $data['row']                    = VisaStatus::where('id', '=', $id)->first();
            if($request->isMethod('post')){
                $postData = $request->all();
                $rules = [
                    'name'           => 'required',
                ...
                if($this->validate($request, $rules)){
                    ...
                    VisaStatus::where($this->data['primary_key'], '=', $id)->update($fields);
                    /* user activity */
                        $activityData = [
                            'user_email'        => session('user_data')['email'],
                            'user_name'         => session('user_data')['name'],
